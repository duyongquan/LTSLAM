...
#include "xslam/opencv/feature_detection/BRIEF.h"
#include "xslam/opencv/random.h"
...
{  
    ...
    for(int i = 0; i < m_bit_length; ++i)
    {
        ...
        do
        {
            x1 = Random::RandomGaussianValue(g_mean, g_sigma);
        } while( x1 > max_v || x1 < -max_v);
        
        do
        {
            y1 = Random::RandomGaussianValue(g_mean, g_sigma);
        ...
        do
        {
            x2 = Random::RandomGaussianValue(meanx, sigma2);
        } while( x2 > max_v || x2 < -max_v);
        
        do
        {
            y2 = Random::RandomGaussianValue(meany, sigma2);
