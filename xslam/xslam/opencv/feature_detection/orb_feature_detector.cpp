...
#include "xslam/opencv/feature_detection/orb_feature_detector.h"
...
{
    //-- 读取图像
    cv::Mat img_1 = cv::imread(image1);
    cv::Mat img_2 = cv::imread(image2);
...
void ORBFeatureDetector::SelfCornerDetect(const std::string& image1, const std::string& image2)
{
    // load image
    cv::Mat first_image = cv::imread(image1, 0);
    cv::Mat second_image = cv::imread(image2, 0);
...
void ORBFeatureDetector::ComputeORB(const cv::Mat &img,
...
void ORBFeatureDetector::BfMatch(const std::vector<DescType> &desc1,
        ...
        std::vector<cv::DMatch> &matches)
{
    ...
    for (size_t i1 = 0; i1 < desc1.size(); ++i1) {
        ...
        for (size_t i2 = 0; i2 < desc2.size(); ++i2) {
            ...
            int distance = 0;
            for (int k = 0; k < 8; k++) {
                distance += _mm_popcnt_u32(static_cast<unsigned int>(desc1[i1][k] ^ desc2[i2][k]));
            }
