#ifndef XSLAM_OPENCV_MATH_VECTOR_HPP
#define XSLAM_OPENCV_MATH_VECTOR_HPP
...
#include "xslam/opencv/math/algorithm.hpp"
...
template <typename T, int N>
inline Vector<T,N>::Vector (T const& v1, T const& v2, T const& v3, T const& v4)
{
...
template <typename T, int N>
template <typename O> inline Vector<T,N>::Vector (Vector<O,N> const& other)
{
    std::copy(*other, *other + N, v);
}
...
template <typename T>
inline Vector<T,3> cross_product (Vector<T,3> const& v1, Vector<T,3> const& v2)
{
...
#endif // XSLAM_OPENCV_MATH_VECTOR_HPP