...
#include "xslam/opencv/geometry_transform/stereo_vision.h"
...
void StereoVision::RunDemo(const std::string& image_left, const std::string& image_right)
{
    ...
    // 基线
    const double b = 0.573;

    // 读取图像
    cv::Mat left  = cv::imread(image_left, 0);
    cv::Mat right = cv::imread(image_right, 0);
    cv::Ptr<cv::StereoSGBM> sgbm = cv::StereoSGBM::create(
            0, 96, 9, 8 * 9 * 9, 32 * 9 * 9,
            1, 63, 10, 100, 32);    // 神奇的参数
...
void StereoVision::ShowPointCloud(
