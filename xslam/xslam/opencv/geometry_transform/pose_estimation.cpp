...
void PoseEstimation::RunDemo1(const std::string& vedio, const std::string& image)
{
    ...
    cv::Mat obj_image = cv::imread(image);
    if (obj_image.empty()) return;
    ...
    if (obj_keypoint.empty() || obj_descriptor.empty()) return;
    fmatcher->add(obj_descriptor);

    // Open a video
    cv::VideoCapture video;
    if (!video.open(vedio)) return;
...
void PoseEstimation::RunDemo2(const std::string& vedio, const std::string& image)
{
    // const char *input = "data/blais.mp4", *cover = "data/blais.jpg";
...
void PoseEstimation::RunDemo3(const std::string& vedio, const std::string& image)
{
    // const char *input = "data/blais.mp4", *cover = "data/blais.jpg";
    size_t min_inlier_num = 100;
