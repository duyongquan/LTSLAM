...
void BundleAdjustmentGlobal::RunDemo(const std::string& filename)
{
    ...
    {
        FILE* fin = fopen(cv::format(filename.c_str(), i).c_str(), "rt");
        if (fin == NULL) return;
        ...
        fclose(fin);
        xs.push_back(pts);
        if (xs.front().size() != xs.back().size()) return;
    }
    ...
    for (size_t j = 0; j < xs.size(); j++)
    {
        ...
        {
            ceres::CostFunction* cost_func = geometry_transform::ReprojectionError::create(xs[j][i], f, cv::Point2d(cx, cy));
