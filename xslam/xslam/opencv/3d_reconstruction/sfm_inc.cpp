...
void SFMInc::RunDemo(const std::string& filename)
{
    double img_resize = 0.25;
    double f_init = 500, cx_init = -1, cy_init = -1, Z_limit = 100;
    double max_cos_parallax = cos(10 * CV_PI / 180), ba_loss_width = 9; // Negative 'loss_width' makes BA not to use a loss function.
    ...
    if (!video.open(filename.c_str())) return;
    ...
    for (size_t i = 0; i < img_set.size(); i++)
    {
        for (size_t j = i + 1; j < img_set.size(); j++)
        {
            ...
            if (show_match)
            {
                ...
                cv::imshow("X-SLAM Tutorial: Structure-from-Motion", match_image);
                cv::waitKey();
            }
        }
    }
    ...
    while (true)
    {
        ...
        {
            printf("X-SLAM Tutorial: There is no good match. Try again after reducing 'max_cos_parallax'.\n");
            return;
        }
        ...
        printf("X-SLAM Tutorial: Image %u - %u were checked as the best match (# of inliers = %zd, # of good points = %d).\n", best_cam0, best_cam1, match_inlier[best_pair].size(), best_score[best_pair]);
    ...
    printf("X-SLAM Tutorial: Image %d - %d are complete (# of 3D points = %zd).\n", best_cam0, best_cam1, Xs.size());
    ...
    while (true)
    {
        ...
        if (pts_3d.size() < 10)
        {
            printf("X-SLAM Tutorial: Image %d is ignored (due to the small number of points).\n", next_cam);
        ...
        if (new_pts_total < 10)
        {
            printf("X-SLAM Tutorial: Image %d is complete (only localization; no 3D point addition).\n", next_cam);
            img_added.insert(next_cam);
            continue;
        }
        ...
        printf("X-SLAM Tutorial: Image %d is complete (# of 3D points = %zd).\n", next_cam, Xs.size());
    ...
    for (size_t j = 0; j < cameras.size(); j++)
        printf("X-SLAM Tutorial: Camera %zd's (f, cx, cy) = (%.3f, %.1f, %.1f)\n", j, cameras[j][6], cameras[j][7], cameras[j][8]);
...
cv::Mat SFMInc::getCameraMat(const SFM::Vec9d& camera)
{
...
cv::Mat SFMInc::getProjectionMat(const SFM::Vec9d& camera)
{
...
void SFMInc::updateCameraPose(SFM::Vec9d& camera, const cv::Mat& R, const cv::Mat& t)
{
    cv::Mat rvec = R.clone();
...
bool SFMInc::isBadPoint(const cv::Point3d& X, const SFM::Vec9d& camera1, const SFM::Vec9d& camera2, double Z_limit, double max_cos_parallax)
{
