...
void PerspectiveCorrection::RunDemo(const std::string& filename)
{
    ...
    cv::namedWindow("X-SLAM Tutorial: Perspective Correction");
    cv::setMouseCallback("X-SLAM Tutorial: Perspective Correction", MouseEventHandler, &points_src);
    while (points_src.size() < 4)
    {
        ...
        cv::imshow("X-SLAM Tutorial: Perspective Correction", display);
        if (cv::waitKey(1) == 27) break; // 'ESC' key: Exit
    }
    if (points_src.size() < 4) return;
    ...
    cv::imshow("X-SLAM Tutorial: Perspective Correction", rectify);
    cv::waitKey(0);
}
