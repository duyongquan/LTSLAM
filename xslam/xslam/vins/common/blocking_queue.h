#ifndef XSLAM_VINS_COMMON_BLOCKING_QUEUE_H_
#define XSLAM_VINS_COMMON_BLOCKING_QUEUE_H_
...
public:
    ...
    bool PushWithTimeout(T t, const common::Duration timeout) 
    {
        ...
        if (!lock.AwaitWithTimeout(
                [this]() { return QueueNotFullCondition(); }, timeout)) {
            return false;
        }
        deque_.push_back(std::move(t));
        return true;
    ...
    {
        MutexLocker lock(&mutex_);
        lock.Await([this]() { return QueueNotEmptyCondition(); });
    ...
    {
        MutexLocker lock(&mutex_);
        if (!lock.AwaitWithTimeout([this]() { return QueueNotEmptyCondition(); }, timeout)) {
            return nullptr;
        }
        T t = std::move(deque_.front());
        deque_.pop_front();
...
private:
    ...
    bool QueueNotEmptyCondition() { return !deque_.empty(); }
...
#endif  // XSLAM_VINS_COMMON_BLOCKING_QUEUE_H_